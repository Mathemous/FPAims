...
export function narrativeHighlights(narrative, query) {
  const terms = normalize(query).split(' ').filter(Boolean).map(token);
  return String(narrative).split(/([\p{L}\p{M}\p{N}]+)/u).filter(Boolean).map(text => ({
    text,
    match: terms.length > 0 && normalize(text).split(' ').filter(Boolean)
      .some(word => terms.some(term => token(word).includes(term))),
  }));
}
